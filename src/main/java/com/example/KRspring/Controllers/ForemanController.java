...
public class ForemanController {
    ...
    public String showNewForemanForm(Model model) {
        ...
        model.addAttribute("objects", objectService.getAllObjects()); // Добавлено для передачи объектов
        return "new_foreman";
    }
    ...
    public String deleteForeman(@PathVariable Long id) {
        // Обновляем все объекты, связанные с прорабом, перед его удалением
        objectService.updateObjectsToNullForeman(id);
        // Удаляем прораба
